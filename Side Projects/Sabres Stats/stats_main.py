...
def format_game_csv(playerDB, path):
    ...
    temp_list = []
    for game_csv in glob.glob(path):
        with open(game_csv, 'r') as f:
            reader = csv.reader(f)
            next(reader)  # skips first row #
            finder = game_csv.find('.')
            gameID = game_csv[finder - 2:finder]
            period = game_csv[(finder) + 1]
            for row in reader:
                i = 0
                while i < len(row):
                    # check = row[i]
...
def writeList(tList):
    ...
    filename = playerPath + str(tList[1]) + ".csv"
    # create new function #
...
## global variables ##
team_roster = "sabres_roster.csv"
# http://www.hockeydb.com/ihdb/stats/leagues/seasons/teams/0000332019.html #
gmPath = "Games/*.csv"
playerPath = "Individual Player Stats/"  ## user defined directory for individual player stats
fileHeader = ["GameID", "PNum", "TOI:1", "TOI:2", "TOI:3", "CF:1", "CF:2", "CF:3", "CA:1", "CA:2", "CA:3", "FF:1",
              "FF:2", "FF:3", "FA:1", "FA:2", "FA:3", "SF:1", "SF:2", "SF:3", "SA:1", "SA:2", "SA:3", "GF:1", "GF:2",
              "GF:3", "GA:1", "GA:2", "GA:3", "SCF:1", "SCF:2", "SCF:3", "SCA:1", "SCA:2", "SCA:3", "HCDF:1", "HCDF:2",
              "HCDF:3", "HCDA:1", "HCDA:2", "HCDA:3"]
